...
export const createEventAction = async (data: CreateEventType) => {
    try {
        ...
        showToast("event created succesfully",'success');
        return response;
    }  catch (error:any) {
...
export const fetchEventData = async (eventType:any,checkIsAdmin?:any, limit = 10, offset = 0) => {
    try {
        ...
        // console.log("/events/display-events",response.data);
...
export const updateEventAction = async (eventId:any, data:any) => {
    try {
        // console.log('API', api.getUri());
        // console.log('API URL:', process.env.NEXT_PUBLIC_BASE_URL);
...
export const deleteEventAction = async (eventId:any) => {
    try {
        ...
        if (response.status === 200) {
            showToast(response.data.message || "Event deleted successfully",'success');
...
export const deleteMediaAction = async (mediaId:any) => {
    try {
        ...
        if (response.status === 200) {
            showToast(response.data.message || "Media deleted successfully",'success');
        } else if (response.status === 404) {
...
export const deleteEventPermanentlyAction = async (eventID:any) => {
    try {
        ...
        if (response.status === 200) {
            showToast(response.data.message || "Event permanently deleted succesfully",'success');
        } else if (response.status === 404) {
            showToast("Medias not found",'error');
        }
...
export const fetchEventRegisteredMembersAction = async (eventID:any,limit?:any,offset?:any) => {
    try {
        const societyId = await AsyncStorage.getItem('societyId'); // Retrieve the societyId from localStorage
