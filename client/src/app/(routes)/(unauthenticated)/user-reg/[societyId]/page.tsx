...
import { getSocietyAction, getInviteUserAction } from '@/src/actions/society';
...
const UserRegistration = ({ params }) => {
    ...
    // const decryptedValues = decryptMobileSociety(params.societyId)
    ...
    useEffect(() => {
        const getSociety = async () => {
            try {
                const data = await getInviteUserAction(params.societyId)
                setUserData(data)
                // Fetch society details using decryptedValues.societyId
                if (data) {
                    const values = await getSocietyAction(data?.societyId);
                                    ...
                                    <TabPanel className="flex flex-col justify-center   items-center ">
                                        {steps === 1 ? (
                                            ...
                                            <>
                                                ...
                                                >
                                                    Enter the OTP sent to {userData?.phoneNumber}
                                                </Typography>
                                                <OTPForm phoneNumber={userData?.phoneNumber} useId={userData?.userId} societyId={userData?.societyId} setSteps={setSteps} />
                                            </>
                                        ) : (
                                            <CreatePasswordForm phoneNumber={userData?.phoneNumber} societyId={userData?.societyId} useId={userData?.userId}
                                            />
                                        )}
                                    </TabPanel>
                                    <TabPanel className="flex flex-col justify-center   items-center ">
